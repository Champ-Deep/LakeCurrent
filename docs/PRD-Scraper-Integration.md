# PRD: LakeCurrent Integration into Lake B2B Scraper
...
The Lake B2B Scraper is domain-focused — a user must already know which domain to scrape. There is no way to go from a question ("insurtech startups using React") to scraped business intelligence. Users must manually find domains, then feed them to the scraper one by one.
